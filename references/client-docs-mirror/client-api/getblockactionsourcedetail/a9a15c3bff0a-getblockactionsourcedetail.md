...
Source: https://open.feishu.cn/document/getBlockActionSourceDetail
Markdown source: https://open.feishu.cn/document/getBlockActionSourceDetail.md
...
&emsp;<br>∟<br>&nbsp;<br>actionTime | number | Action发生的时间戳、单位秒
&emsp;<br>∟<br>&nbsp;<br>messages | object[] | message列表，最多可返回 20 条数据。
&emsp;&emsp;<br>∟<br>&nbsp;<br>messageType | string | 消息类型<br>**可选值**：<br>- `text`：文本消息<br>- `image`：图片消息<br>- `post`：富文本消息<br>- `media`：视频消息，支持版本[3.47+](https://open.feishu.cn/document/uYjL24iN/uAjMuAjMuAjM/version-compatibility)<br>- `file`：文件消息，支持版本[3.47+](https://open.feishu.cn/document/uYjL24iN/uAjMuAjMuAjM/version-compatibility)<br>- `interactive`：[消息卡片](https://open.feishu.cn/document/ukTMukTMukTM/uczM3QjL3MzN04yNzcDN)消息，支持版本[3.47+](https://open.feishu.cn/document/uYjL24iN/uAjMuAjMuAjM/version-compatibility)<br>- `unsupport`：暂不支持的消息类型
&emsp;&emsp;<br>∟<br>&nbsp;<br>sender | object | message发送者对象
&emsp;&emsp;&emsp;&emsp;<br>∟<br>&nbsp;<br>name | string | message发送者名字，按调用时客户端语言提供
&emsp;&emsp;&emsp;&emsp;<br>∟<br>&nbsp;<br>open_id | string | 用户 [open_id](https://open.feishu.cn/document/home/user-identity-introduction/open-id)
&emsp;&emsp;<br>∟<br>&nbsp;<br>createTime | number | message创建的时间戳、单位秒
&emsp;&emsp;<br>∟<br>&nbsp;<br>support | boolean | 是否是支持的消息类型
&emsp;&emsp;<br>∟<br>&nbsp;<br>content | string | 消息内容：json字符串、文本内容
&emsp;&emsp;<br>∟<br>&nbsp;<br>status | boolean | 消息状态是否有效
&emsp;&emsp;<br>∟<br>&nbsp;<br>openChatId | string | 触发操作会话的  [open_chat_id](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-id-description)<br>**注意事项**：飞书[V3.40.0](https://open.feishu.cn/document/uYjL24iN/uAjMuAjMuAjM/version-compatibility)及以上版本支持
&emsp;&emsp;<br>∟<br>&nbsp;<br>openMessageId | string | 触发操作的消息 [open_message_id](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/intro#ac79c1c2)<br>**注意事项**：飞书[V3.40.0](https://open.feishu.cn/document/uYjL24iN/uAjMuAjMuAjM/version-compatibility)及以上版本支持
...
**3. Postman实例截图**
![image.png](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/99b472fcf8ff06cedbafbdc5196cd2c9_soybkeJbRu.png?height=1031&lazyload=true&width=3814)
