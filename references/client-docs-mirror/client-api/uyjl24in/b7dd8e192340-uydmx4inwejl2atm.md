...
Source: https://open.feishu.cn/document/uYjL24iN/uYDMx4iNwEjL2ATM
Markdown source: https://open.feishu.cn/document/uYjL24iN/uYDMx4iNwEjL2ATM.md
...
名称 | 数据类型 | 是否必填 | 默认值 | 描述
---|---|---|---|---
url | string | 是 | \- | 开发者服务器地址。<br>**注意事项**：**注意**：仅支持 HTTP/HTTPS 协议。
filePath | string | 是 | \- | 要上传文件资源的路径 (本地路径)。
name | string | 是 | \- | 文件对应的 key，开发者在服务端可以通过这个 key 获取文件的二进制内容。
header | object | 否 | \- | HTTP 的请求头（Header）。Header 内 content-type 为 `multipart/form-data`，不可变更。<br>**注意事项**：**注意**：`header` 不支持设置 `referer`。
...
名称 | 数据类型 | 描述
---|---|---
trace | string | 请求 ID，用于请求问题的诊断和全链路追踪。<br>**注意事项**：**注意**：飞书 [V4.7.0](https://open.feishu.cn/document/uYjL24iN/uAjMuAjMuAjM/version-compatibility) 及以上版本支持输出该字段。
