...
Source: https://open.feishu.cn/document/uYjL24iN/ucDMx4yNwEjL3ATM
Markdown source: https://open.feishu.cn/document/uYjL24iN/ucDMx4yNwEjL3ATM.md
...
filePath | string | 否 | \- | 指定文件下载后存储的路径 (本地路径)。格式示例为 `ttfile://temp/feishu.png` 或 `ttfile://user/feishu.png`，其中 `ttfile://temp/` 和 `ttfile://user/` 为下载目录，`feishu.png` 为文件名。更多详情可参见[文件系统](https://open.feishu.cn/document/uYjL24iN/uETOuETOuETO/file-system)。<br>- 默认的下载目录为 `ttfile://temp/`<br>- 示例值：ttfile://temp/feishu.png<br>**注意事项**：**注意**：飞书 [V3.43.0](https://open.feishu.cn/document/uYjL24iN/uAjMuAjMuAjM/version-compatibility) 及以上版本支持设置该字段。
