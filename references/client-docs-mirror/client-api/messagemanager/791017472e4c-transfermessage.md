...
Source: https://open.feishu.cn/document/uYjL24iN/uQjMuQjMuQjM/web-view-component-capability/MessageManager/transferMessage
Markdown source: https://open.feishu.cn/document/uYjL24iN/uQjMuQjMuQjM/web-view-component-capability/MessageManager/transferMessage.md
...
名称 | 数据类型 | 是否必填 | 默认值 | 描述
---|---|---|---|---
data | object | 是 | {} | 需要传输的消息体。<br>**示例值**：{"test":"testValue"}
channel | object | 是 | \- | 发送消息过程中需要指定消息通道，一个通道唯一对应一个 web-view 与小程序 worker 间的映射。channel 来自 web-view 的 bindload 回调中的 `e.detail.channel`。<br>**示例值**："MESSAGE_CHANNEL_5"<br>**注意事项**：**注意**：从 web-view 向小程序发送消息时，不支持设置该参数。
