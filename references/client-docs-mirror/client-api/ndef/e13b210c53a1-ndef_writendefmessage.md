...
Source: https://open.feishu.cn/document/uYjL24iN/uQzM4YjL0MDO24CNzgjN/ndef/ndef_writendefmessage
Markdown source: https://open.feishu.cn/document/uYjL24iN/uQzM4YjL0MDO24CNzgjN/ndef/ndef_writendefmessage.md
Generated by scripts/crawl_client_docs.py.
-->
# Ndef.writeNdefMessage(Object object)

写入数据到 NDEF 类型标签
**注意事项**：如果扫描到NDEF-Formatable类型的标签
...
---|---|---|---|---|---
小程序 | V7.48.0+ | V7.48.0+ | **X** | V7.51.0+ | 预览
...
名称 | 数据类型 | 必填 | 默认值 | 描述
---|---|---|---|---
uris | string[] | 否 | &nbsp; | URI类型数组<br>**注意事项**：**注意**：当传入非URI类型的数据时，会被转换成文本类型写入
texts | string[] | 否 | &nbsp; | 文本类型数组<br>**注意事项**：**注意**：文本类型数据会在句首拼接当前的语言的LanguageCode
records | object[] | 否 | &nbsp; | NdefRecord类型数组
&emsp;<br>∟<br>&nbsp;<br>id | arraybuffer | 否 | &nbsp; | NdefRecord的id值。示例值：new Uint8Array([0x30, 0x03]).buffer
