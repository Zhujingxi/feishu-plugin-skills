...
Source: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/application-v6/application/patch
Markdown source: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/application-v6/application/patch.md
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 更新应用分组(admin:app.category:update)<br>更新应用信息(application:application)
...
callback_type | string | 否 | 回调类型<br>**示例值**："webhook"<br>**可选值有**：<br>- webhook：将回调发送至开发者服务器<br>- websocket：使用长连接接收回调
