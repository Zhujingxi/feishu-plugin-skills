...
Source: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/task-v2/section/patch
Markdown source: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/task-v2/section/patch.md
...
目前支持更新的字段包括：
* `name` - 自定义字段名字;
* `insert_before` - 要让当前自定义分组放到某个自定义分组前面的section_guid，用于改变当前自定义分组的位置;
...
name | string | 否 | 自定义分组名。如更新，不允许设为空，支持最大100个UTF-8字符。<br>**示例值**："已经审核过的任务"
...
update_fields | string\[\] | 是 | 要更新的字段名，支持：<br>* `name` - 自定义字段名字<br>* `insert_before` - 要让当前自定义分组放到某个自定义分组前面的section_guid，用于改变当前自定义分组的位置。<br>* `insert_after` - 要让当前自定义分组放到某个自定义分组后面的secion_guid，用于改变当前自定义分组的位置。<br>**示例值**：["name"]<br>**数据校验规则**：<br>- 长度范围：`1` ～ `10`
