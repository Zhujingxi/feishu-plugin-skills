...
Source: https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/board-v1/whiteboard-node/list
Markdown source: https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/board-v1/whiteboard-node/list.md
...
top_length | number(float) | 梯形上底长（短边）。默认为下底边长度的2/3，不能超过下底边长度。
cube | cube | 六面体属性，type=cube时可以设置
control_point | point | 六面体控制点，相对六面体外接矩形的相对坐标。默认控制点为外接矩形长的0.8，宽的0.25
...
specified_coordinate | boolean | 指定连线坐标及长宽。为 true 时需要用户设置连线的坐标及长宽信息。为 false 时会根据连线的开始、结束端点自动计算连线的坐标及长宽信息
caption_position_type | int | 文字相对连线的位置类型，0=OnLine，表示文字在连线那不；1=AboveLine，文字在连线的上方，文字在线的上方，连线走向的上边或左边；2=BellowLine，文字在连线的下方，文字在线的上方，连线走向的下边或右边<br>**可选值有**：<br>- 0：文字在线内<br>- 1：文字在线的上方，连线走向的上边或左边<br>- 2：文字在线的下方，连线走向的下边或右边
...
syntax_type | int | 语法类别<br>**可选值有**：<br>- 0：未知<br>- 1：Plantuml解析<br>- 2：Mermaid解析<br>- 3：SVG 语法解析
code | string | PlantUml/Mermaid源码
style_type | int | 语法样式（画板样式/经典样式）<br>**可选值有**：<br>- 1：画板样式<br>- 2：经典样式
