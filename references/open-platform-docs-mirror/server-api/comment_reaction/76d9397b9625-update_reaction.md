...
Source: https://open.feishu.cn/document/ukTMukTMukTM/uIzNzUjLyczM14iM3MTN/drive-v2/comment_reaction/update_reaction
Markdown source: https://open.feishu.cn/document/ukTMukTMukTM/uIzNzUjLyczM14iM3MTN/drive-v2/comment_reaction/update_reaction.md
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 添加、回复云文档中的评论(docs:document.comment:create)<br>回复、修改、删除云文档中的评论(docs:document.comment:write_only)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
file_type | string | 是 | 文件类型，用于区分不同类型的云文档，可选值需参考[开放平台文件类型枚举](https://open.feishu.cn/document/ukTMukTMukTM/uczNzUjL3czM14yN3MTN)<br>**示例值**：docx<br>**数据校验规则**：<br>- 长度范围：`1` ～ `100` 字符

### 请求体

名称 | 类型 | 必填 | 描述
---|---|---|---
action | string | 是 | 操作类型<br>**示例值**："add"<br>**可选值有**：<br>- add：添加表情回复<br>- delete：删除添加的reaction<br>**数据校验规则**：<br>- 长度范围：`1` ～ `100` 字符
