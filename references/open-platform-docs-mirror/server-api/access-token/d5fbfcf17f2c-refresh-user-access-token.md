...
Source: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/refresh-user-access-token
Markdown source: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/refresh-user-access-token.md
Generated by scripts/crawl_client_docs.py.
-->
# 刷新 user_access_token（v2 版本）
**注意事项**：本接口已成为历史版本，不推荐使用。请使用最新版本：[刷新 user_access_token](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/refresh-user-access-token-v3)。
...
**注意事项**：刷新后请更新本地`refresh_token`，原`refresh_token`将无法再使用。原 `user_access_token` 不受影响，在其过期时间到达前仍可正常使用。
...
refresh_token | string | 用于刷新 `user_access_token`，该字段仅在请求成功且用户授予 `offline_access` 权限时返回：<br>offline_access(offline_access)<br>**注意事项**：`refresh_token` 仅能被使用一次。
