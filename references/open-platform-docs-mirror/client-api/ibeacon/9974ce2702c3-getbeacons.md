...
Source: https://open.feishu.cn/document/uYjL24iN/uQTOuQTOuQTO/ibeacon/getbeacons
Markdown source: https://open.feishu.cn/document/uYjL24iN/uQTOuQTOuQTO/ibeacon/getbeacons.md
...
&emsp;<br>∟<br>&nbsp;<br>uuid | string | Beacon 设备广播的 uuid。
&emsp;<br>∟<br>&nbsp;<br>major | number | Beacon 设备的主 id。<br>**注意事项**：**注意**：历史存在 Android 中 `major` 以及 `minor` 字段为 string 类型的问题，该问题在飞书 V5.26.0 版本已修复。
&emsp;<br>∟<br>&nbsp;<br>minor | number | Beacon 设备的次 id。<br>**注意事项**：**注意**：历史存在 Android 中 `major` 以及 `minor` 字段为 string 类型的问题，该问题在飞书 V5.26.0 版本已修复。
&emsp;<br>∟<br>&nbsp;<br>proximity | number | 表示设备距离的枚举值。可能值：<br>- `0`：无效<br>- `1`：非常近<br>- `2`：近<br>- `3`：远
&emsp;<br>∟<br>&nbsp;<br>accuracy | number | Beacon 设备的距离。
&emsp;<br>∟<br>&nbsp;<br>rssi | number | 表示设备的信号强度。
